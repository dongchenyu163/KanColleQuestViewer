...
function drawPlate(ptObj, num)
{
	...
	//
	ctx.beginPath();
	ctx.rect(0, 0, w * 0.7, ptObj.height + delta);
	//
	var plateGrad = ctx.createLinearGradient(0, 0, w * 0.7, 0);
	...
	ctx.fillRect(0, 0, w * 0.7, ptObj.height + delta);
	//
	ctx.globalAlpha = 1;
    ...
    //
    var plateGrad2 = ctx.createLinearGradient(0, 0, w * 0.7, 0);
	...
	ctx.shadowOffsetY = 1;
	//
	ctx.translate(w * 0.7 - w * 0.35, ptObj.height / 2);	
	//
	drawTPText(ctx, num);	
	//
	ctx.restore();
