...
function drawShip(picObj)
{
	...
	{
		smokeCount = 0;
		addSmoke(img_ship.x - 318, -30, 0.4, 0.5, 0.8);
		addSmoke(img_ship.x - 370, -30, 0.4, 0.5, 0.8);
		//
		if(shipLifes < 0.6)
		{
			addSmoke(img_ship.x - Math.random() * img_ship.pic.width / 2 - img_ship.pic.width / 4 - 170, -10, 0.3, 0.4, 0.4);
			addSmoke(img_ship.x - Math.random() * img_ship.pic.width / 2 - img_ship.pic.width / 4 - 170, -10, 0.3, 0.4, 0.4);
		}
		//
		if(shipLifes < 0.2)
	...
	ctx.lineTo(- w / 2 + picObj.x- picObj.pic.width / 2 + 341, picObj.y);
	ctx.lineTo(- w / 2 + picObj.x- picObj.pic.width / 2 + 339, picObj.y + 8);
	...
	//
	ctx.beginPath();
	ctx.rect(picObj.x - picObj.pic.width / 2 - 200, - picObj.pic.height + 20, 67 * shipLifes, 5);
    ctx.fill();
    //
    ctx.beginPath();
    ctx.rect(picObj.x - picObj.pic.width / 2 - 200, - picObj.pic.height + 20, 67, 5);
    ctx.stroke();
	...
	//
	ctx.drawImage(wavePic, - w / 2 + picObj.x - 283, 0);

	///////////FIRE
	if(shipLifes < 1)
	{
		addFire(img_ship.x - Math.random() * img_ship.pic.width / 2 - img_ship.pic.width / 4 - 170, -10 + Math.random() * 8, 0.8 * (1 - shipLifes), 0.4 * (1 - shipLifes));
		//
...
function hitTestShip(bul_x, bul_y)
{
	...
	{
		if(bul_x < 100 && bul_y < horizLine + img_island.y)
		{
			//
		}
		else
