...
function init()
{
	...
	Button.y = h - 46;
	//
	Pattern.bPattern.height = 150;
	Pattern.tPattern.alpha = 1;
	Pattern.bPattern.alpha = 0.8;
	drawPatternElements();
...
function update(time)
{
	...
	if((autoplayFl === true || interactFl === true) && (icoText != 0 || shipLifes > 0))
	{
		...
		if(interactFl === true)
		{
			...
			ctx2.save();
			ctx2.translate(1, h - 2);
			ctx2.beginPath();
			ctx2.rect(0, 0, (w - 2) * koef, 1);
			ctx2.fillStyle = "red";
			ctx2.fill();
			ctx2.restore();
		}
...
function goAutoplayScenario()
{
	...
	TweenMax.to(img_ico,   0.8, {x:     680, ease:Power3.easeInOut, delay:0.2});
	TweenMax.to(img_plate, 0.6, {y:   h + 1, ease:Power2.easeOut             });
	//
	TweenMax.delayedCall(1.2, mClick, [null]);
	TweenMax.to(img_ico, 0.8, {x: 680 + 80, ease:Power3.easeInOut, delay:1.5});
	//
...
function goPackshot()
{
	...
	//
	TweenMax.to(img_plate, 0.6, {y:   h + 1, ease:Power2.easeOut});
	TweenMax.to(img_ico, 0.8, {x: 680 + 80, ease:Power3.easeInOut});
	//
...
function mClick(evt)
{
	if(img_cam.isShoot === false && bannerState === "interaction" && (allowFl === true || evt === null))
	{
		...
		//
		addBullet(w / 2 - 550, h + 100, img_aim.x - Math.random() * 10, img_aim.y + amplY * Math.sin(img_aim.ang));
		addBullet(w / 2 + 550, h + 100, img_aim.x + Math.random() * 10, img_aim.y + amplY * Math.sin(img_aim.ang));
		addBullet(w / 2 - 370, h + 100, img_aim.x - Math.random() * 10, img_aim.y + amplY * Math.sin(img_aim.ang));
		addBullet(w / 2 + 370, h + 100, img_aim.x + Math.random() * 10, img_aim.y + amplY * Math.sin(img_aim.ang));
		addBullet(w / 2 - 50,  h + 100, img_aim.x - Math.random() * 5,  img_aim.y + amplY * Math.sin(img_aim.ang));
		addBullet(w / 2 + 50,  h + 100, img_aim.x + Math.random() * 5,  img_aim.y + amplY * Math.sin(img_aim.ang));
	}
